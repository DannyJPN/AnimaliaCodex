...
public static class ToDone
{
      ...
      IOptions<PermissionOptions> permissionOptions)
  {
    ...
    var canExecute = entryActionsResolver.CanExecuteAction(entry, JournalActionCodes.CLOSE);
    ...
    using (var tx = await dbContext.Database.BeginTransactionAsync())
    {
      ...
      switch (entry.Status)
      {
        ...
        case JournalRecordStatuses.REVIEW_DOCUMENTATION_DEP:
          ...
          entry.ArchiveReviewNote = request.ReviewNote;
          entry.Status = JournalRecordStatuses.CLOSED_IN_DOCUMENTATION_DEP;
          break;
        default:
          // No status change for other statuses
          break;
      }
