...
public class InZooBulk
{
  public record Request(int ClassId, string Mode);
  ...
  {
    Enum.TryParse<InZooFilterEnum>(request.Mode, true, out var modeEnum);

    var supportedStatuses = modeEnum == InZooFilterEnum.NoEuPermit || modeEnum == InZooFilterEnum.NoEuPermitRegOnly
      ? new[] { "Z" }
      : new[] { "Z", "D" };
    ...
    switch (modeEnum)
    {
      case InZooFilterEnum.None:
        break;
      case InZooFilterEnum.Reg:
        query = query.Where(s => s.IsRegulationRequirement);
        break;
      case InZooFilterEnum.Eufauna:
        query = query.Where(s => s.IsEuFauna && string.IsNullOrEmpty(s.ProtectionTypeCode));
        break;
      case InZooFilterEnum.CrProtection:
        query = query.Where(s => !string.IsNullOrEmpty(s.ProtectionTypeCode));
        break;
      case InZooFilterEnum.NoEuPermit:
        query = query.Where(s => s.EuCode == "A");
        break;
      case InZooFilterEnum.NoEuPermitRegOnly:
        query = query.Where(s => s.IsRegulationRequirement && s.EuCode == "A");
        break;
      case InZooFilterEnum.DecisionEuFauna:
        query = query.Where(s => !string.IsNullOrEmpty(s.EuFaunaRefNumber));
        break;
      case InZooFilterEnum.DecisionCrException:
        query = query.Where(s => !string.IsNullOrEmpty(s.CrExceptionRefNumber));
        break;
