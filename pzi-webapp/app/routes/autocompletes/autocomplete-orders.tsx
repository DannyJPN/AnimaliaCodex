...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const [latResults, czResults] = await Promise.all([
    fetchODataList<OrderItem>(queryLatClauses.join('&')),
    fetchODataList<OrderItem>(queryCzClauses.join('&'))
  ]);
  ...
  const mergedItems = new Map<number, OrderItem>();
  ...
  const orderResults: SelectItemType<number, string>[] = Array.from(mergedItems.values())
    .slice(0, 15)
    .map((item) => ({
      key: item.id,
  ...
  return Response.json(orderResults);
}
