...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const [latResults, czResults] = await Promise.all([
    fetchODataList<FamilyItem>(queryLatClauses.join('&')),
    fetchODataList<FamilyItem>(queryCzClauses.join('&'))
  ]);
  ...
  const mergedItems = new Map<number, FamilyItem>();
  ...
  const familyResults: SelectItemType<number, string>[] = Array.from(mergedItems.values())
    .slice(0, 15)
    .map((item) => ({
      key: item.id,
  ...
  return Response.json(familyResults);
}
