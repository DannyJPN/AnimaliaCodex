...
export interface ClassStatistics {
  ...
  citesSpeciesCount: number;
  citesSpecimensCount: number;
  euSpeciesCount: number;
  euSpecimensCount: number;
  ...
  for (const cls of response.classStatistics) {
    blockData.push({
      ...
      data: {
        ...
        'CITESd': cls.citesSpeciesCount,
        'CITESj': cls.citesSpecimensCount,
        'EUd': cls.euSpeciesCount,
        'EUj': cls.euSpecimensCount,
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const templateBlocks = await prepareTemplateBlocks('statistika__statistika_eep.xlsx', 'statistika_eep');
  ...
  return new Response(xlsxBuffer, {
    status: 200,
    headers: {
      "Content-Disposition": `inline;filename=statistika-eep-${getXlsFileTimestamp()}.xlsx`,
