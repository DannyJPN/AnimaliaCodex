      ...
      for (const dataRow of dataRows) {
        blockData.push({
          ...
          data: {
            ...
            "RegPovinnost": dataRow.isRegulationRequirement ? "RP" : "",
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const apiResponse = await apiCall(
    "api/PrintExports/InZooBulkNoEuPermitRegOnly",
    "POST",
  ...
  const filenameSuffix = `-no-eu-permit-reg-only-${getXlsFileTimestamp()}`;

  return new Response(xlsxBuffer, {
    status: 200,
    headers: {
      "Content-Disposition": `inline;filename=in-zoo${filenameSuffix}.xlsx`,
