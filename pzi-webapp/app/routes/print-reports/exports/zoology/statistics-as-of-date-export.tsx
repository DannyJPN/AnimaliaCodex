/* Sestavy / Programy: Statistika ke dni */
...
export function toExportBlocks(response: StatisticsResponse, asOfDate: string): BlockData[] {
...
const parsedResponse = await processResponse<StatisticsResponse>(apiResponse);
  const responseData = parsedResponse.item || {} as StatisticsResponse;

  const dataBlocks = toExportBlocks(responseData, date);

  const templateBlocks = await prepareTemplateBlocks('statistika__statistika.xlsx', 'statistika');
  ...
  return new Response(xlsxBuffer, {
    status: 200,
    headers: {
      "Content-Disposition": `inline;filename=w_dStatistika_${getXlsFileTimestamp()}.xlsx`,
