/* ZA-102 -> Sestavy/Programy - Ekonomika - Zabavené exempláře - všechny (obratlovci) */
...
export default function SeizedSpecimensAllReport() {
  ...
  const handleDownload = async () => {
    ...
    const fileUrl = `/print-reports/economy/seized-specimens-all-export?${new URLSearchParams(params).toString()}`;
      ...
      <Card className="rounded-none border bg-card text-card-foreground shadow-none">
        <fieldset disabled={false} className="flex flex-wrap gap-4 p-2 bg-secondary min-h-[50px]">
          <CardTitle className="text-xl p-4">
            {report.type} - {report.name} (všechny)
          </CardTitle>
        </fieldset>
