...
export default function CREvidenceByMovementDate() {
  const { report } = useLoaderData<typeof loader>() as LoaderData;
  const [dateFrom, setDateFrom] = useState<string>('');
  const [dateTo, setDateTo] = useState<string>('');
  const [mode, setMode] = useState<string>('crprotection');
  ...
  const handleDownload = async () => {
    ...
    await downloadFile(`/print-reports/zoology/cr-evidence-by-movement-date?${new URLSearchParams(params as any).toString()}`);
  };
      ...
      <Card className="rounded-none border bg-card text-card-foreground shadow-none">
        ...
        <div>
          <CardContent className="p-2 pt-0 space-y-4">
            ...
            <TextField name="modeSelect" className="w-full">
              <Label>Režim</Label>
              <Select 
                selectedKey={mode}
                defaultSelectedKey="crprotection"
                onSelectionChange={(value) => setMode(value as string)}
                aria-label="Režim evidence"
                isRequired
              >
                <SelectTrigger>
                  <SelectValue />
                ...
                <SelectPopover>
                  <SelectListBox>
                    <SelectItem id="crprotection">ČR ochrana</SelectItem>
                    <SelectItem id="eufauna">EU fauna</SelectItem>
                    <SelectItem id="eufaunareduced">EU fauna red.</SelectItem>
                  </SelectListBox>
                </SelectPopover>
              </Select>
              <FieldError />
