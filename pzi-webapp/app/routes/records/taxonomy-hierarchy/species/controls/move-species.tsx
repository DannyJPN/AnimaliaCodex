...
import { action as moveSpeciesAction } from '../operations/move-species';

export function MoveSpecies(props: {
  selectedIds: number[],
  currentParentId: number
  onClose: () => void
}) {
  const submitFetcher = useFetcher<typeof moveSpeciesAction>();
  ...
  const parentQuery = useQuery({
    queryKey: ['genera-search', parentSearchQueryDebounced, props.currentParentId],
  ...
  return (
    <DialogOverlay
      ...
      <DialogContent
        ...
        <submitFetcher.Form
          method="post"
          action="/records/species/operations/move-species"
          className="grid grid-cols-1">

          {props.selectedIds.map((id, idx) => {
            return (
              <input key={id} type="hidden" name={`speciesIds[${idx}]`} value={id} />
            );
          })}
