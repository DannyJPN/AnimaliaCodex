...
import { action as moveOrdersAction } from '../operations/move-orders';

export function MoveOrders(props: {
  selectedIds: number[],
  currentParentId: number
  onClose: () => void
}) {
  const submitFetcher = useFetcher<typeof moveOrdersAction>();
  ...
  const parentQuery = useQuery({
    queryKey: ['classes-search', parentSearchQueryDebounced, props.currentParentId],
  ...
  return (
    <DialogOverlay
      ...
      <DialogContent
        ...
        <submitFetcher.Form
          method="post"
          action="/records/orders/operations/move-orders"
          className="grid grid-cols-1">

          {props.selectedIds.map((id, idx) => {
            return (
              <input key={id} type="hidden" name={`orderIds[${idx}]`} value={id} />
            );
          })}
