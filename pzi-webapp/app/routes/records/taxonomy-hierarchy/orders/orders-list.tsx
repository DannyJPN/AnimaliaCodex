...
import { MoveOrders } from "./controls/move-orders";
import { TaxonomyClassInfoItem, TaxonomyOrderItem } from "./models";
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const filterClauses = [
    `$filter=taxonomyClassId eq ${classId}`
  ];
  ...
  const [fetchError, listResult] = await fetchODataList<TaxonomyOrderItem>(
    `TaxonomyOrders?$count=true&$orderby=code&${filterClauses.join(' and ')}`
  );

  const [classFetchError, classResult] = await fetchODataList<TaxonomyClassInfoItem>(
    `taxonomyclasses?$filter=id eq ${classId}`
  );

  const [phylaFetchError, phylaResult] = await fetchODataList<TaxonomyPhylumInfoItem>(
    `taxonomyphyla?$filter=id eq ${classResult?.items[0].taxonomyPhylumId}`
  );
...
export default function List() {
  ...
  const breadcrumbProps = {
    ...
    levels: {
      ...
      class: {
        id: loaderData.classInfo!.id,
        nameLat: loaderData.classInfo!.nameLat,
        nameCz: loaderData.classInfo!.nameCz
      },
      currentPage: {
        nameLat: 'Ordo',
        nameCz: 'Řád'
  ...
  const gridOptions: MRT_TableOptions<TaxonomyOrderItem> = {
    columns: columnDef,
    ...
    muiTableBodyRowProps: ({ row }) => {
      ...
      return {
        ...
        onClick: () => {
          navigate(`/records/classes/${loaderData.classId}/orders/${row.original.id}${location.search}`, { replace: true });
        },
        ...
        onDoubleClick: () => {
          navigate(`/records/orders/${row.original.id}/families/`, { replace: true });
        },
  ...
  const openNewItemForm = () => {
    navigate(`/records/classes/${loaderData.classId}/orders/new${location.search}`, { replace: true });
  };
  ...
  useEffect(() => {
    ...
    let timeout = setTimeout(() => {
      if (loaderData.items.length === 0) {
        ...
        navigate(`/records/classes/${loaderData.classId}/orders/${loaderData.items[0].id}${location.search}`, { replace: true });
      }
    }, 10);
  ...
  const handleExport = async () => {
    const url = getExportXlsUrl(
      `/records/classes/${loaderData.classId}/orders/export-xls`,
      searchParams
    );
    ...
    <div className={cn("flex-1 md:flex", '')}>
      ...
      {moveItemsVisible && (
        <MoveOrders
          ...
          currentParentId={loaderData.classId}
