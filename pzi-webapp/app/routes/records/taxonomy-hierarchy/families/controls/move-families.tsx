...
import { action as moveFamiliesAction } from '../operations/move-families';

export function MoveFamilies(props: {
  selectedIds: number[],
  currentParentId: number
  onClose: () => void
}) {
  const submitFetcher = useFetcher<typeof moveFamiliesAction>();
  ...
  const parentQuery = useQuery({
    queryKey: ['orders-search', parentSearchQueryDebounced, props.currentParentId],
  ...
  return (
    <DialogOverlay
      ...
      <DialogContent
        ...
        <submitFetcher.Form
          method="post"
          action="/records/families/operations/move-families"
          className="grid grid-cols-1">

          {props.selectedIds.map((id, idx) => {
            return (
              <input key={id} type="hidden" name={`familyIds[${idx}]`} value={id} />
            );
          })}
