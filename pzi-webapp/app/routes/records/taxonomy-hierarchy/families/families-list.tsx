...
import { MoveFamilies } from "./controls/move-families";
import { TaxonomyFamilyItem, TaxonomyOrderInfoItem } from "./models";
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const filterClauses = [
    `$filter=taxonomyOrderId eq ${parentId}`
  ];
  ...
  const [fetchError, listResult] = await fetchODataList<TaxonomyFamilyItem>(
    `taxonomyfamilies?$count=true&$orderby=code&${filterClauses.join(' and ')}`
  );

  const [orderFetchError, orderResult] = await fetchODataList<TaxonomyOrderInfoItem>(
    `taxonomyorders?$filter=id eq ${parentId}&$expand=taxonomyClass($select=id,taxonomyPhylumId,nameLat,nameCz)`
  );

  const [phylaFetchError, phylaResult] = await fetchODataList<TaxonomyPhylumInfoItem>(
    `taxonomyphyla?$filter=id eq ${orderResult?.items[0].taxonomyClass?.taxonomyPhylumId}`
  );
  ...
  return data({
    orderId: parentId,
    orderInfo: orderResult?.items[0],
    phylumId: phylaResult?.items[0].id,
...
export default function List() {
  ...
  const breadcrumbProps = {
    ...
    levels: {
      ...
      class: {
        id: loaderData.orderInfo!.taxonomyClass!.id,
        nameLat: loaderData.orderInfo!.taxonomyClass!.nameLat,
        nameCz: loaderData.orderInfo!.taxonomyClass!.nameCz
      },
      order: {
        id: loaderData.orderInfo!.id,
        nameLat: loaderData.orderInfo!.nameLat,
        nameCz: loaderData.orderInfo!.nameCz
      },
      currentPage: {
        nameLat: 'Familia',
        nameCz: 'Čeleď'
  ...
  const gridOptions: MRT_TableOptions<TaxonomyFamilyItem> = {
    columns: columnDef,
    ...
    muiTableBodyRowProps: ({ row }) => {
      ...
      return {
        ...
        onClick: () => {
          navigate(`/records/orders/${loaderData.orderId}/families/${row.original.id}${location.search}`, { replace: true });
        },
        ...
        onDoubleClick: () => {
          navigate(`/records/families/${row.original.id}/genera/`, { replace: true });
        },
  ...
  const openNewItemForm = () => {
    navigate(`/records/orders/${loaderData.orderId}/families/new${location.search}`, { replace: true });
  };
  ...
  useEffect(() => {
    ...
    let timeout = setTimeout(() => {
      if (loaderData.items.length === 0) {
        ...
        navigate(`/records/orders/${loaderData.orderId}/families/${loaderData.items[0].id}${location.search}`, { replace: true });
      }
    }, 10);
  ...
  const handleExport = async () => {
    const url = getExportXlsUrl(
      `/records/orders/${loaderData.orderId}/families/export-xls`,
      searchParams
    );
    ...
    <div className={cn("flex-1 md:flex", '')}>
      ...
      {moveItemsVisible && (
        <MoveFamilies
          ...
          currentParentId={loaderData.orderId}
          onClose={() => {
