...
export async function action({ request }: ActionFunctionArgs) {
  return await handleCUD<TaxonomyPhylumItem>(
    request,
    (formData) => {
      const postData: Partial<TaxonomyPhylumItem> = Object.fromEntries(formData);
...
export default function Detail() {
  ...
  const [selectedItem, setSelectedItem] = useState<TaxonomyPhylumItem | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    switch (fetcher.data?.action) {
      case 'delete': {
        navigate(`/records/phyla${location.search}`);
        break;
      }
      case 'insert': {
        navigate(`/records/phyla/${fetcher.data.changeResult?.id}${location.search}`);
        break;
      }
    }
      ...
      <fetcher.Form method="POST" className="flex flex-col h-full">
        <FormValidationContext.Provider value={fetcher.data?.validationErrors || {}}>
          ...
          <ItemListNavigation
            ...
            getItemLink={(itm) => {
              return `/records/phyla/${itm.id}${location.search}`
            }} />
      ...
      <MenuPopover>
        <Menu>
          <MenuSection>
            ...
            <MenuItem href={`/records/phyla/${itemId}/classes`}>Třídy</MenuItem>
          </MenuSection>
        </Menu>
      </MenuPopover>
    </MenuTrigger>
