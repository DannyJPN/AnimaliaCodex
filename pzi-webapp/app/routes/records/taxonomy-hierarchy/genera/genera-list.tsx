...
import { MoveGenera } from "./controls/move-genera";
import { TaxonomyFamilyInfoItem, TaxonomyGenusItem } from "./models";
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const filterClauses = [
    `$filter=taxonomyFamilyId eq ${parentId}`
  ];
  ...
  const [fetchError, listResult] = await fetchODataList<TaxonomyGenusItem>(
    `taxonomygenera?$count=true&$orderby=code&${filterClauses.join(' and ')}`
  );

  const [parentFetchError, parentResult] = await fetchODataList<TaxonomyFamilyInfoItem>(
    `taxonomyfamilies?$filter=id eq ${parentId}&$expand=taxonomyOrder($select=id,nameLat,nameCz;$expand=taxonomyClass($select=id,taxonomyPhylumId,nameLat,nameCz))`
  );

  const [phylaFetchError, phylaResult] = await fetchODataList<TaxonomyPhylumInfoItem>(
    `taxonomyphyla?$filter=id eq ${parentResult?.items[0].taxonomyOrder?.taxonomyClass?.taxonomyPhylumId}`
  );
  ...
  return data({
    familyId: parentId,
    familyInfo: parentResult?.items[0],
...
export default function List() {
  ...
  const gridOptions: MRT_TableOptions<TaxonomyGenusItem> = {
    columns: columnDef,
    ...
    muiTableBodyRowProps: ({ row }) => {
      ...
      return {
        ...
        onClick: () => {
          navigate(`/records/families/${loaderData.familyId}/genera/${row.original.id}${location.search}`, { replace: true });
        },
        ...
        onDoubleClick: () => {
          navigate(`/records/genera/${row.original.id}/species/`, { replace: true });
        },
  ...
  const openNewItemForm = () => {
    navigate(`/records/families/${loaderData.familyId}/genera/new${location.search}`, { replace: true });
  };
  ...
  useEffect(() => {
    ...
    let timeout = setTimeout(() => {
      if (loaderData.items.length === 0) {
        ...
        navigate(`/records/families/${loaderData.familyId}/genera/${loaderData.items[0].id}${location.search}`, { replace: true });
      }
    }, 10);
  ...
  const handleExport = async () => {
    const url = getExportXlsUrl(
      `/records/families/${loaderData.familyId}/genera/export-xls`,
      searchParams
    );
    ...
    <div className={cn("flex-1 md:flex", '')}>
      ...
      {moveItemsVisible && (
        <MoveGenera
          ...
          currentParentId={loaderData.familyId}
          onClose={() => {
