...
export async function action({ request }: ActionFunctionArgs) {
  return await handleCUD<ExpositionArea>(
    request,
    (formData) => {
      const postData: Partial<ExpositionArea> = Object.fromEntries(formData);
      return postData;
    },
    'api/expositionareas',
    pziConfig
...
export default function Detail() {
  ...
  const [selectedItem, setSelectedItem] = useState<ExpositionArea | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    switch (fetcher.data?.action) {
      case 'delete': {
        navigate(`/records/exposition-hierarchy/areas${location.search}`);
        break;
      }
      case 'insert': {
        navigate(`/records/exposition-hierarchy/areas/${fetcher.data.changeResult?.id}${location.search}`);
        break;
      }
    }
