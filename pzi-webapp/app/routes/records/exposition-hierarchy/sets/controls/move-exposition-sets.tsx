...
import { action as moveClassesAction } from '../operations/move-exposition-sets';

export function MoveExpositionSets(props: {
  selectedIds: number[],
  ...
  const parentQuery = useQuery({
    queryKey: ['expositionSet-search', parentSearchQueryDebounced, props.currentParentId],
      ...
      const result = await fetchJson<SelectItemType<number, string>[]>(
        '/autocompletes/autocomplete-exposition-areas',
        {
          ...
          body: JSON.stringify({
            ...
            ignoredIds: [props.currentParentId, ...props.selectedIds]
          }),
          signal: signal
        }
      );
  ...
  return (
    <DialogOverlay
      ...
      <DialogContent
        ...
        <submitFetcher.Form
          method="post"
          action="/records/sets/operations/move-exposition-sets"
