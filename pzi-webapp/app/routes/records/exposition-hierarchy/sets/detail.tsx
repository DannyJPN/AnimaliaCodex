...
export async function action({ request }: ActionFunctionArgs) {
  return await handleCUD<ExpositionSet>(
    request,
    (formData) => {
      const postData: Partial<ExpositionSet> = Object.fromEntries(formData);
      return postData;
    },
    'api/expositionsets',
    pziConfig
...
export default function Detail() {
  ...
  const [selectedItem, setSelectedItem] = useState<ExpositionSet | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    switch (fetcher.data?.action) {
      case 'delete': {
        navigate(`/records/exposition-hierarchy/areas/${params.parentId}/sets${location.search}`);
        break;
      }
      case 'insert': {
        navigate(`/records/exposition-hierarchy/areas/${params.parentId}/sets/${fetcher.data.changeResult?.id}${location.search}`);
        break;
      }
    }
  ...
  useEffect(() => {
    if (itemId !== Number.MIN_SAFE_INTEGER) {
      ...
      setSelectedItem({
        ...
        expositionAreaId: parseInt(params.parentId!)
      } as ExpositionSet);
    }

    setFormKey(Date.now().toString());
  }, [itemId, outletContext.items, params.parentId]);
      ...
      <fetcher.Form method="POST" className="flex flex-col h-full">
        <FormValidationContext.Provider value={fetcher.data?.validationErrors || {}}>
          ...
          <fieldset
            ...
            <input type='hidden' name='expositionAreaId' defaultValue={selectedItem.expositionAreaId} />
          </fieldset>
