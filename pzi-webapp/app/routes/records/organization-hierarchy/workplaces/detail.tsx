...
export default function Detail() {
    ...
    useEffect(() => {
        ...
        switch (fetcher.data?.action) {
            case 'delete': {
                navigate(`/records/org-hierarchy/departments/${outletContext.departmentId}/workplaces${location.search}`);
                break;
            }
            case 'insert': {
                navigate(`/records/org-hierarchy/departments/${outletContext.departmentId}/workplaces/${fetcher.data.changeResult?.id}${location.search}`, { replace: true });
                break;
            }
        }
    ...
    useEffect(() => {
        ...
        if (itemId !== Number.MIN_SAFE_INTEGER && foundItem) {
            ...
            setSelectedItem({
                ...
                parentId: outletContext.departmentId
            ...
            <fetcher.Form method="POST" className="flex flex-col h-full">
                <FormValidationContext.Provider value={fetcher.data?.validationErrors || {}}>
                    ...
                    <ItemListNavigation
                        ...
                        getItemLink={(itm) => {
                            return `/records/org-hierarchy/departments/${outletContext.departmentId}/workplaces/${itm.id}${location.search}`
                        }} />
