...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const [listError, listResult] = await fetchODataList<CadaverPartner>(
    `cadaverpartners?$count=true&orderby=id desc`
  );
  ...
  const tableSettings = await fetchTableSettings(userName!, CADAVER_PARTNERS_TABLE_ID, {
...
const postTableSettings = createPostTableSettingsHandler(CADAVER_PARTNERS_TABLE_ID);
...
export default function List() {
  ...
  const goToDetail = (id: number) => {
    navigate(`/lists/cadaver/${id}${location.search}`, { replace: true });
  }

  const openNewItemForm = () => {
    navigate(`/lists/cadaver/new${location.search}`, { replace: true });
  };

  const { table, rowVirtualizerInstanceRef } = useConfiguredTable<CadaverPartner>({
    searchParams,
    setSearchParams,
  ...
  useDefaultTableRedirects<CadaverPartner>({
  ...
  const handleDownload = async () => {
    ...
    var fileUrl = `/lists/cadaver/export-xls${location.search}`;
