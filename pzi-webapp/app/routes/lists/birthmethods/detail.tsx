...
import { BirthMethod } from "./models";
...
export async function action({ request }: ActionFunctionArgs) {
    return await handleStringCUD<BirthMethod>(
        request,
        (formData) => {
            const formDataEntries = Object.fromEntries(formData);
            const postData: Partial<BirthMethod> = {
...
export default function Detail() {
  ...
  const [selectedItem, setSelectedItem] = useState<BirthMethod | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    switch (fetcher.data?.action) {
      case 'delete': {
        navigate(`/lists/birthmethods${location.search}`);
        break;
      }
      case 'insert': {
        navigate(`/lists/birthmethods/${fetcher.data.changeResult?.code}${location.search}`);
        break;
      }
    }
      ...
      <fetcher.Form method="POST" className="flex flex-col h-full">
        <FormValidationContext.Provider value={fetcher.data?.validationErrors || {}}>
          ...
          <ItemListNavigation
                      ...
                      getItemLink={(itm) => {
                        return `/lists/birthmethods/${itm.code}${location.search}`
                      }} />
