...
import { columnDef, columnDefVisibility, BIRTHMETHODS_TABLE_ID } from "./controls";
...
// use MaterialReactTable with BirthMethod columns; override getRowId via tableOptions
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const [listError, listResult] = await fetchODataList<EnumItem>(
    `birthmethods?$count=true&$orderby=code`
  );
  ...
  const tableSettings = await fetchTableSettings(userName!, BIRTHMETHODS_TABLE_ID, {
...
const postTableSettings = createPostTableSettingsHandler(BIRTHMETHODS_TABLE_ID);
...
export default function List() {
  ...
  const goToDetail = (code: string) => {
    navigate(`/lists/birthmethods/${code}${location.search}`, { replace: true });
  }

  const openNewItemForm = () => {
    navigate(`/lists/birthmethods/new${location.search}`, { replace: true });
  };
            ...
            <div className="@container">
              <div className="min-h-[72px] w-full content-center flex-wrap p-2 bg-secondary">
                <Select
                  ...
                  defaultSelectedKey="birthmethods"
